...
const CATEGORY_ICON: Record<string, string> = {
  cash: '💵', bank: '🏦', stocks: '📈', rrsp: '🇨🇦',
}

export default function App() {
  const validTabs: Tab[] = ['dashboard', 'savings', 'mortgage', 'income']
  const hashTab = window.location.hash.replace('#', '') as Tab
  const [activeTab, setActiveTab] = useState<Tab>(validTabs.includes(hashTab) ? hashTab : 'dashboard')
  ...
  const { rates, loading: ratesLoading, offline, refresh } = useFxRates(primary)
  ...
  const amountById = useMemo(() => {
    ...
    for (const s of filtered) {
      map.set(s.id!, rates ? convert(s.amount, s.currency, primary, rates) : s.amount)
    }
    return map
  ...
  async function handleAdd(data: Omit<SavingEntry, 'id' | 'createdAt' | 'updatedAt'>) {
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col">
      ...
      <header className="sticky top-0 z-40 bg-slate-950/90 backdrop-blur border-b border-slate-800 px-4 py-3 flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <span className="text-xl font-bold tracking-tight">💰 Save My Finances</span>
          {offline && (
            <span className="text-xs px-2 py-0.5 rounded-full bg-amber-900 text-amber-300 font-semibold">Offline</span>
          )}
        </div>
      ...
      <nav className="bg-slate-950 border-b border-slate-800 flex">
        {([
          ...
          { id: 'income',    label: '💸 Income & Expenses', short: '💸\nIncome' },
          { id: 'mortgage',  label: '📐 Mortgage',         short: '📐\nMortgage' },
        ] as { id: Tab; label: string; short: string }[]).map(({ id, label, short }) => (
          <button
            key={id}
            onClick={() => navigate(id)}
        ...
        {activeTab === 'savings' && (
          <>
            ...
            <div className="mb-6 p-5 rounded-2xl bg-slate-900 border border-slate-800 flex items-center justify-between gap-4">
              <div>
                <p className="text-xs text-slate-400 uppercase tracking-widest mb-1">Total ({primary})</p>
                <p className="text-2xl sm:text-4xl font-bold text-white tabular-nums">{formatMoney(totalInPrimary, primary)}</p>
                {ratesLoading && <p className="text-xs text-slate-500 mt-1">Updating exchange rates…</p>}
              </div>
              <div className="hidden sm:flex flex-col gap-1 text-right">
                {(['cash', 'bank', 'stocks', 'rrsp'] as FilterCategory[]).map((cat) => {
                  const catTotal = savings
                    .filter((s) => s.category === cat)
                    .reduce((sum, s) => sum + (rates ? convert(s.amount, s.currency, primary, rates) : s.amount), 0)
                  if (catTotal === 0) return null
                  return (
                    <p key={cat} className="text-xs text-slate-400">
                      <span className="mr-1">{CATEGORY_ICON[cat]}</span>
                      <span className="font-semibold text-slate-200 tabular-nums">{formatMoney(catTotal, primary)}</span>
            ...
            <div className="flex gap-2 mb-6 flex-wrap">
              {FILTERS.map((f) => (
                ...
                >
                  {f !== 'all' && <span className="mr-1">{CATEGORY_ICON[f]}</span>}
                  {f.toUpperCase()}
                </button>
              ))}
            </div>
            ...
            {showAdd && (
              <div className="mb-6 p-5 rounded-2xl bg-slate-900 border border-slate-700">
                ...
                <SavingForm onSave={handleAdd} onCancel={() => setShowAdd(false)} />
              </div>
            )}
            ...
            {filtered.length === 0 ? (
              ...
              <div className="flex flex-col gap-8">
                {[...groups.entries()].map(([groupName, entries]) => {
                  const groupTotal = entries.reduce((sum, s) => sum + (amountById.get(s.id!) ?? 0), 0)
                  return (
                    <section key={groupName}>
                      <div className="flex items-center justify-between mb-3 pb-2 border-b border-slate-800">
                        <div className="flex items-center gap-2">
                          <span className="text-lg font-bold text-slate-100">{groupName}</span>
                      ...
                      <div className="grid grid-cols-2 sm:flex sm:flex-wrap gap-3 sm:gap-4">
                        {entries.map((entry) => (
                          <SavingCard
                            ...
                            primaryCurrency={primary}
                            rates={rates}
                            weight={(amountById.get(entry.id!) ?? 0) / maxAmount}
                          />
                        ))}
                      </div>
                    </section>
        ...
        {activeTab === 'mortgage' && (
          <>
            ...
            <div className="mb-5 p-4 rounded-2xl bg-slate-900 border border-slate-800 flex items-center justify-between gap-4">
              <div>
                <p className="text-xs text-slate-400 uppercase tracking-widest mb-0.5">Available Savings ({primary})</p>
                <p className="text-2xl font-bold text-emerald-300 tabular-nums">{formatMoney(totalInPrimary, primary)}</p>
              </div>
              <div className="hidden sm:flex flex-col gap-1 text-right">
                {(['cash', 'bank', 'stocks', 'rrsp'] as FilterCategory[]).map((cat) => {
                  const catTotal = savings
            ...
            <MortgageCalculator monthlyIncome={monthlyIncome} monthlyExpenses={monthlyExpenses} />
          </>
        )}
      </main>
